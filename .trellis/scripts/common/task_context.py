...
def cmd_init_context(args: argparse.Namespace) -> int:
    ...
    if not is_monorepo(repo_root):
        ...
        if not validate_package(cli_package, repo_root):
            ...
            print(
                colored(f"Error: unknown package '{cli_package}'. Available: {available}", Colors.RED), file=sys.stderr
            )
            return 1
        package = cli_package
    else:
    ...
    print('     python3 task.py add-context <dir> implement <spec-path> "<reason>"')
