...
def get_submodule_packages(repo_root: Path | None = None) -> dict[str, str]:
    ...
    if packages is None:
        return {}
    return {name: cfg.get("path", name) for name, cfg in packages.items() if cfg.get("type") == "submodule"}
...
def get_git_packages(repo_root: Path | None = None) -> dict[str, str]:
    ...
    if packages is None:
        return {}
    return {name: cfg.get("path", name) for name, cfg in packages.items() if _is_true_config_value(cfg.get("git"))}
