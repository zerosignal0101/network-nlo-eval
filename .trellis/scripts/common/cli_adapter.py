...
class CLIAdapter:
    ...
    def build_run_command(
        ...
        if self.platform == "opencode":
            ...
            raise ValueError("Antigravity workflows are UI slash commands; CLI agent run is not supported.")
        elif self.platform == "windsurf":
            raise ValueError("Windsurf workflows are UI slash commands; CLI agent run is not supported.")
        elif self.platform == "qoder":
            cmd = ["qodercli", "-p", prompt]
        elif self.platform == "codebuddy":
            raise ValueError("CodeBuddy does not support non-interactive mode (no CLI agent)")
        elif self.platform == "copilot":
            raise ValueError("GitHub Copilot is IDE-only; CLI agent run is not supported.")
        elif self.platform == "droid":
            raise ValueError("Factory Droid CLI agent run is not yet integrated with Trellis multi-agent.")
    ...
    def build_resume_command(self, session_id: str) -> list[str]:
        ...
        if self.platform == "opencode":
            ...
            raise ValueError("Antigravity workflows are UI slash commands; CLI resume is not supported.")
        elif self.platform == "windsurf":
            raise ValueError("Windsurf workflows are UI slash commands; CLI resume is not supported.")
        elif self.platform == "qoder":
            ...
            raise ValueError("Factory Droid CLI resume is not yet integrated with Trellis multi-agent.")
        else:
...
def _has_other_platform_dir(project_root: Path, exclude: set[str]) -> bool:
    ...
    return any((project_root / d).is_dir() for d in _ALL_PLATFORM_CONFIG_DIRS if d not in exclude)
...
def detect_platform(project_root: Path) -> Platform:
    ...
    if (project_root / ".codex").is_dir() and not _has_other_platform_dir(project_root, {".codex", ".agents"}):
        return "codex"
    ...
    if (project_root / ".kiro" / "skills").is_dir() and not _has_other_platform_dir(project_root, {".kiro"}):
        return "kiro"
    ...
    if (project_root / ".windsurf" / "workflows").is_dir() and not _has_other_platform_dir(project_root, {".windsurf"}):
        return "windsurf"
