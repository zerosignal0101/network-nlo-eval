...
_SUBMODULE_SQUASH_WARNING_MARKER = "Merge submodule PR(s) first. If squash-merged, update submodule ref after merge."
...
def _get_submodule_default_branch(submodule_abs: Path) -> str:
    ...
    """
    ret, out, _ = run_git(["symbolic-ref", "refs/remotes/origin/HEAD"], cwd=submodule_abs)
...
def _process_submodule_changes(
    ...
    for pkg_name, pkg_path in submodule_packages.items():
        ...
        ret, status_out, _ = run_git(["status", "--porcelain"], cwd=sub_abs)
        if ret != 0:
            continue
        if status_out.strip():
    ...
    for pkg_name, pkg_path in changed:
        ...
        if ret == 0:
            ...
            ret, _, err = run_git(["checkout", sub_branch], cwd=sub_abs)
            if ret != 0:
            ...
            if ret_anc != 0:
                print(f"  {Colors.YELLOW}[WARN] submodule branch has diverged history, consider recreating{Colors.NC}")
        else:
            # Create new branch
            ret, _, err = run_git(["checkout", "-b", sub_branch], cwd=sub_abs)
            if ret != 0:
        ...
        if ret != 0:
            # Has staged changes
            ret, _, err = run_git(["commit", "-m", sub_commit_msg], cwd=sub_abs)
            if ret != 0:
        ...
        # --- Push ---
        ret, _, err = run_git(["push", "-u", "origin", sub_branch], cwd=sub_abs)
        if ret != 0:
        ...
        if existing_sub_pr:
            ...
            result = subprocess.run(
                [
                    ...
                    sub_base,
                    "--title",
                    f"{commit_prefix}({scope}): {task_name} [{pkg_name}]",
                    "--body",
                    f"Submodule changes for {task_name}",
                ],
                capture_output=True,
                text=True,
                encoding="utf-8",
            ...
            if result.returncode != 0:
                print(f"{Colors.RED}Failed to create PR for {pkg_name}: {result.stderr}{Colors.NC}")
                return submodule_prs, changed_paths, False
...
def _build_submodule_warning(submodule_prs: dict[str, str]) -> str:
    ...
    return f"> {_SUBMODULE_SQUASH_WARNING_MARKER}\n>\n> Submodule PRs:\n{pr_lines}\n\n---\n\n"
...
def main() -> int:
    ...
    if not sub_success:
        print(f"\n{Colors.RED}Submodule PR flow failed. Skipping main repo commit/PR.{Colors.NC}")
    ...
    if not has_staged_changes:
        ...
        ret, log_out, _ = run_git(["log", f"origin/{current_branch}..HEAD", "--oneline"])
    ...
    if args.dry_run:
        ...
        if existing_pr:
            ...
            if has_submodule_prs:
                _ensure_submodule_warning_on_existing_pr(submodule_prs, args.dry_run)
        else:
            # Read PRD as PR body
            pr_body = ""
            prd_file = target_dir_path / "prd.md"
