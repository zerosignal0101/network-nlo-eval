...
from common.log import Colors, log_error, log_info, log_success, log_warn
...
def _init_submodules_for_task(task_data: dict, worktree_path: str, project_root: Path) -> None:
    ...
    if task_package and isinstance(task_package, str):
        if validate_package(task_package, project_root):
            ...
            log_warn(f"package '{task_package}' not found in config.yaml, skipping submodule init")
            return
    else:
        ...
        if default_pkg:
            if validate_package(default_pkg, project_root):
                ...
                log_warn(f"package '{default_pkg}' not found in config.yaml, skipping submodule init")
                return
    ...
    ret, status_out, status_err = run_git(["submodule", "status", submodule_path], cwd=Path(worktree_path))

    if ret != 0:
        log_warn(f"git submodule status failed for '{submodule_path}': {status_err.strip()}, skipping submodule init")
        return
    ...
    if prefix == "-":
        ...
        log_warn(f"submodule {submodule_path} has local changes, skipping update")
    elif prefix == "U":
        log_warn(f"submodule {submodule_path} has conflicts, skipping")
    else:
        log_warn(f"Unknown submodule status prefix '{prefix}' for '{submodule_path}', skipping")
...
def main() -> int:
    ...
    if not worktree_path or not Path(worktree_path).is_dir():
        ...
        _, base_branch_out, _ = run_git(["branch", "--show-current"], cwd=project_root)
        ...
        if ret == 0:
            ...
            ret, _, err = run_git(["worktree", "add", worktree_path, branch], cwd=project_root)
        else:
            log_info(f"Creating new branch: {branch}")
            ret, _, err = run_git(["worktree", "add", "-b", branch, worktree_path], cwd=project_root)
    ...
    registry_add_agent(task_id, worktree_path, agent_pid, task_dir_relative, project_root, platform)
