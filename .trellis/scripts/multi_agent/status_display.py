...
def cmd_summary(repo_root: Path, filter_assignee: str | None = None) -> int:
    ...
    from .status_monitor import get_last_message, get_last_tool
    ...
    registry_data = read_json(registry_file) if registry_file and registry_file.is_file() else None
    ...
    if running_tasks:
        ...
        for t in running_tasks:
            priority_color = (
                Colors.RED if t["priority"] == "P0" else (Colors.YELLOW if t["priority"] == "P1" else Colors.BLUE)
            )
            print(
