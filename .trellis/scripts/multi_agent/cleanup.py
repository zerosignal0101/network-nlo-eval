...
def cleanup_worktree(branch: str, repo_root: Path, skip_confirm: bool, keep_branch: bool) -> int:
    """Cleanup single worktree."""
    # Find worktree path for branch
    _, worktree_list, _ = run_git(["worktree", "list", "--porcelain"], cwd=repo_root)
    ...
    ret, _, _ = run_git(["worktree", "remove", worktree_path, "--force"], cwd=repo_root)
    if ret != 0:
...
def cmd_merged(repo_root: Path, skip_confirm: bool, keep_branch: bool) -> int:
    ...
    # Get main branch
    _, head_out, _ = run_git(["symbolic-ref", "refs/remotes/origin/HEAD"], cwd=repo_root)
    ...
    # Get merged branches
    _, merged_out, _ = run_git(["branch", "--merged", main_branch], cwd=repo_root)
    merged_branches = []
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Multi-Agent Pipeline: Cleanup Worktree")
